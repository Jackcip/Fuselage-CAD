...
# CENTRAL BODY
outer_body = cq.Workplane("XY").circle(rad).extrude(-length)

inner_body = cq.Workplane("XY").circle(rad - thickness).extrude(-length)
...
sketch_cut = cq.Workplane("XZ").spline(inner_pts).lineTo(0, 0).close()
